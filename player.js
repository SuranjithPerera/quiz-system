    ...
    try {
        ...
        if (currentState) { //
            switch (currentState.status) { //
                ...
                case 'finished': //
                    await showFinalResults(); // Call showFinalResults which is now async
...
async function onGameStateChange(gameState) { // Made async to await showFinalResults
    ...
    switch (gameState.status) { //
        ...
        case 'finished': //
            ...
            await showFinalResults(); // Await the updated async function
            break; //
...
async function showFinalResults() {
    ...
    if (playerGamePlayerId && currentGamePin && typeof database !== 'undefined' && database.ref) { // Check database.ref
        try {
            const playerRef = database.ref(`games/${currentGamePin}/players/${playerGamePlayerId}`); //
            ...
            if (playerData && typeof playerData.score === 'number') { //
                ...
                playerScore = finalScoreForDisplay; // Ensure global playerScore is also synced //
                console.log('PLAYER: Fetched definitive final score:', finalScoreForDisplay); //
            }
        } catch (err) {
            console.error('PLAYER: Error fetching definitive final score, using current global playerScore:', err); //
        }
    }
    ...
    // Also update the #current-score element if it exists, though it's on a hidden screen
    const scoreEl = document.getElementById('current-score'); //
    if (scoreEl) { //
        scoreEl.textContent = finalScoreForDisplay; // Use definitive score
    }
    
    showStatus('🏁 Quiz completed! Final results are in!', 'success'); //

    // Detach Firebase listeners BEFORE clearing local state
    if (playerGameInstance) { //
        playerGameInstance.cleanup(); // This should call database.ref(...).off() //
    }
    
    clearPlayerState(); // Now clear global state variables (playerScore will become 0 etc.) //
